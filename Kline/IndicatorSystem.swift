...
extension ChartSeries {
    ...
    static func xs(closes: [Double], volumes: [Double], n: Int) -> (sup: [Double], sdn: [Double], lup: [Double], ldn: [Double]) {
        let count = closes.count
        var var2 = [Double](repeating: 0, count: count)
        for i in 0..<count { var2[i] = closes[i] * volumes[i] }
        // VAR2:=CLOSE*VOL; 各周期量价 EMA 比值
        func vratio(_ k: Int) -> [Double] {
            let ev2 = ema(values: var2, period: k)
            let evol = ema(values: volumes, period: k)
            var out = [Double](repeating: 0, count: count)
            for i in 0..<count { out[i] = evol[i] == 0 ? 0 : ev2[i] / evol[i] }
            return out
        }
        let r3 = vratio(3), r6 = vratio(6), r12 = vratio(12), r24 = vratio(24)
        var avg = [Double](repeating: 0, count: count)
        for i in 0..<count { avg[i] = (r3[i] + r6[i] + r12[i] + r24[i]) / 4 }
        // VAR3:=EMA((EMA(VAR2,3)/EMA(VOL,3)+...)/4,N);
        let var3 = ema(values: avg, period: n)
        // SUP:1.06*VAR3; SDN:VAR3*0.94;
        let sup = var3.map { $0 * 1.06 }
        let sdn = var3.map { $0 * 0.94 }
        // VAR4:=EMA(CLOSE,9); LUP:EMA(VAR4*1.14,5); LDN:EMA(VAR4*0.86,5);
        let var4 = ema(values: closes, period: 9)
        let lup = ema(values: var4.map { $0 * 1.14 }, period: 5)
        let ldn = ema(values: var4.map { $0 * 0.86 }, period: 5)
        return (sup, sdn, lup, ldn)
    }

    // MARK: XS2（薛斯通道 II）

    static func dma(_ values: [Double], a: [Double]) -> [Double] {
        var out = Array(repeating: Double.nan, count: values.count)
        for i in 0..<values.count {
            let w = a[i]
            if i == 0 { out[i] = values[i] }
            else if !out[i - 1].isNaN { out[i] = (1 - w) * out[i - 1] + w * values[i] }
            else { out[i] = values[i] }
        }
        return out
    }

    static func xs2(closes: [Double], highs: [Double], lows: [Double], n: Int, m: Int) -> (c1: [Double], c2: [Double], c3: [Double], c4: [Double]) {
        let count = closes.count
        var price = Array(repeating: 0.0, count: count)
        for i in 0..<count { price[i] = (2 * closes[i] + highs[i] + lows[i]) / 4 }
        let aa = ma(values: price, period: 5)
        let c1 = aa.map { $0 * Double(n) / 100 }
        let c2 = aa.map { $0 * Double(200 - n) / 100 }
        let ma20 = ma(values: closes, period: 20)
        var cc = Array(repeating: 0.0, count: count)
        for i in 0..<count {
            let denom = ma20[i]
            cc[i] = (denom.isNaN || denom == 0) ? 0 : abs(price[i] - denom) / denom
        }
        let dd = dma(closes, a: cc)
        let c3 = dd.map { (1 + Double(m) / 100) * $0 }
        let c4 = dd.map { (1 - Double(m) / 100) * $0 }
        return (c1, c2, c3, c4)
    }
}
