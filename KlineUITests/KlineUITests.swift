...
final class KlineUITests: XCTestCase {
    ...
    func test01_Launch_ShowsBottomMenu() throws {
        ...
        // 启动后默认进入「测试2」页（ContentView selectedTab = 5），LOGO 可见即视为启动完成
        XCTAssertTrue(app.staticTexts["test2.logo"].waitForExistence(timeout: 15),
                      "启动后应显示测试2页（默认首页）")

        // 底部 6 个菜单按钮全部存在
        for id in ["tab.home", "tab.favorites", "tab.market", "tab.simulation", "tab.test", "tab.test2"] {
    ...
    func test02_TabSwitching_ShowsEachPage() throws {
        ...
        // 测试
        app.buttons["tab.test"].tap()
        XCTAssertTrue(app.staticTexts["test.marketOverview"].waitForExistence(timeout: 10), "测试页未显示")

        // 测试2
        app.buttons["tab.test2"].tap()
        XCTAssertTrue(app.staticTexts["test2.logo"].waitForExistence(timeout: 10), "测试2页未显示")
    }
