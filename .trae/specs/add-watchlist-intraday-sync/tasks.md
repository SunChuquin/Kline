...
- [ ] Task 8: 【实测留档】全量 vs 差分 的体积与耗时
  - [ ] SubTask 8.1: 实测并记录：① 全量推送（`/upload` 流式）实际秒数 ② 常态差分（含复权重灌）包体积与实际秒数 ③ 设备侧合并实际毫秒数
  - [ ] SubTask 8.2: 把实测值回填到 spec「答疑」表，替换估算并标注「实测」
  - 验收：三个数字均有原始输出留档
