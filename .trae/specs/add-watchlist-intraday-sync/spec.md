...
| 实测吞吐 | **6~11 MB/s**（记录值 2-4 分钟） | `migrate_tdxdb.py:7,59`；`.trae/documents/Kline-数据源改版-可行性分析与实施计划.md:14,57` |
...
| 单行成本（估算） | ≈ **80~100 B/行**（1,379.7 MB ÷ 含周/月/季/年约 1,780 万行量级） | 推算 |
...
| 场景 | 数据量 | 推送耗时（按 5~10 MB/s） |
| :--- | :--- | :--- |
| ① 纯追加当天 1 根（3,611 行） | ≈ **0.4~0.6 MB** | **< 1 秒** |
| ② 常态差分（①+ 22~24 只整段重灌 ≈ 8.5 万行） | ≈ **8~12 MB** | **2~3 秒** |
| ③ 分红季峰值（①+ 上百只重灌） | ≈ **40~60 MB** | **5~12 秒** |
| 对比：全量 | 1,379.7 MB | 2~4 分钟 + 重启 |

**结论：差分把每天要传的量从 1.38 GB 降到约 10 MB（约 1/120），耗时从分钟级降到秒级**；代价是无法用「全量替换」的粗暴方式，必须维护一套差分包契约。
