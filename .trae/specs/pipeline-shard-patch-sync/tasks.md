...
- [x] Task 9: 【实测与调参】—— 已完成（真机）
  ...
  - **本轮暴露的真实短板（已解决）**：commit 的 12s —— 真凶不是热刷新（0ms）也不是关连接（8ms），
        而是 **SQLite 默认 `wal_autocheckpoint=1000页` 在 COMMIT 语句内部对 214.6MB WAL 做全量回写**。
        修法：会话连接设 `PRAGMA wal_autocheckpoint=0`（**13,829ms → 633ms**）+ 响应之后用独立短连接
        `wal_checkpoint(PASSIVE)`。已用「临时改 `SH#600519` 末日 close → commit 后 0.9s 用 App 自身连接
        读回新值」**决定性验证** WAL 语义下 App 不会读到旧值。

#### 追加实测（修 commit 后，热缓存，N=2/W=2）

| 轮次 | 端到端 | PC 出包 | PUT | APPLY | commit |
| :--- | ---: | ---: | ---: | ---: | ---: |
| 冷 | 56.1s | 18.2s | 6.7s | 20.8s | 1.7s |
| 热 1 | 38.8s | 17.3s | 6.7s | 11.3s | 0.7s |
| **热 2（最佳）** | **32.7s** | 14.7s | 6.2s | 8.1s | **0.7s** |

**最终：端到端 57.8s → 32.7s（1.77×）**。剩余构成：PC 出包 ~15s + 设备 PUT ~6s + 设备 APPLY ~8s（部分重叠）。
**设备端仍是单写者下界**，继续加并行度无意义。

**追加：延迟回写的兜底循环（已落地并验证）**
- `deferredWALCheckpoint()`：主循环 `PASSIVE → log==checkpointed → TRUNCATE`，**上限 30 次 / 60s / 间隔 2s**；
  另加**延迟段**（「已回写、只差截断」时等 60s 再试 3 次）。合计 ≤33 次、≤~160s，**全程后台、不阻塞任何链路**。
- `PRAGMA journal_size_limit=64MB`（连接级，两处都设）
- **收敛实测**：`begin 0.0MB → commit 107.2/214.3/322.0MB → 回写后 0.0/0.0/0.0MB`，落盘 `tdx.db-wal=0.0KB`，
  **无单调增长**（修正前 321.9 → 429.2 → 536.8 递增）
- 两个被实测否掉的坑：① **`busy_timeout` 有害**（等待中的 TRUNCATE 让下一轮 `BEGIN IMMEDIATE` 报
  `database is locked`，第 3 连跑中断）→ 改「立即失败 + 有界重试」；② **`log=-4096` 是 SQLite 返回 (-1,-1)**，
  曾被误判为「已回写」→ 判据改为 `log >= 0 && log <= checkpointed`
- 背靠背 3 次：端到端 **43.7 / 34.4 / 34.1s**，commit **0.7 / 0.8 / 0.8s**（无回退），行数 3 次均 863,627
- 我复核那一次：`会话 begin ... WAL=0.0MB`（上轮已回写干净）、commit **621ms**

**端到端耗时区间（须知悉）**：热缓存 **32.7~34.4s**，冷缓存可到 43.7~56.1s。
波动来自 PC 出包（13~23s）与设备 I/O（APPLY 8~21s），**不是**功能问题。

**新增副作用（须知悉）**：`verify_main_db.py` 拉的是**主库文件**，现在**必须等延迟回写（约 13~120s）完成后再拉**，
否则拿到的是尚在 WAL 里的数据（App 侧不受影响，App 读 WAL）。
