...
- [ ] Task 9: 【实测与调参】—— **阻塞：设备锁屏离线**
  - [ ] SubTask 9.1: 跑 N=4/W=4，记录总耗时与分段耗时（对照串行基线 57.8s）
  - [ ] SubTask 9.2: 再跑 (N=2,W=2) 与 (N=6,W=6)，选**稳定且最快**的组合
  - [ ] SubTask 9.3: **手动跑 `verify_main_db.py` 全量核对**
  - [ ] SubTask 9.4: **观察设备日志**在长事务窗口内有无 `SQLITE_BUSY` / 写失败 / 界面异常
  - [ ] SubTask 9.5: 实测数字写入 `tasks.md` 与 `.trae/documents/live-sync/` 对应篇
  - **阻塞原因**：`build_and_deploy.py` 返回 exit 6（`🔒 设备无人值守：iPad 锁屏或 Kline 未在前台`）——
    **设备上跑的还是旧版**（v1.0.2 (378) / (377)），新端点（会话、五表、流式下载）都还没装上去。
    需要：解锁 iPad → 打开 Kline 前台 → 重新跑 `build_and_deploy.py` → 再跑本任务。
...
| 待部署 | 上述 379 构建**已成功**，但 `build_and_deploy.py` 返回 exit 6（设备锁屏未连接）→ **尚未装到设备** |

## 实测对照

| 方案 | 总耗时 | PC 出包 | PUT | APPLY | 备注 |
| :--- | ---: | ---: | ---: | ---: | :--- |
| 串行基线（单包，仅日/周/月） | **57.8s** | 25.4 | 5.9 | 21.5 | 已实测（上一轮） |
| 单包（含季/年，本机 6 核） | PC 侧 **20.8s** | 9.5(A)+6.7(B)+4.5(C) | — | — | A 段曾被首版实现抬到 59.8s，已修回 |
| **并行出包 4 片/4 worker** | PC 侧 **15.4s** | — | — | — | vs 单包 31.8s、同 4 片串行 36.7s → **2.4×** |
| 流水线 N=4/W=4 端到端 | — | — | — | — | **待设备解锁** |
| 流水线 N=2/W=2 | — | | | | |
| 流水线 N=6/W=6 | — | | | | |
