...
- [x] Task 2: 新按钮触摸状态机（B' 交互 + 环 Z' 落点接管，不接图表）
  ...
  - [x] SubTask 2.2: 点击 B' → 果冻回弹（同参数），**不弹面板** —— ⚠️ **该条已变更**：最终定为三步「按需清全部光标 → 窗口朝更新方向平移 1 根 → 在平移后的最右缘放光标并发布联动光标」，仍不开面板；见 `spec.md` 变更记录
  - [x] SubTask 2.3: 闲置 3s 淡出至 25% 且 **C' 隐藏**；触碰即回 100%；令牌自增防泄漏 —— ⚠️ **时长已变更**：新按钮改为 0.5 秒（`wheelIdleFadeDelay`），见 `spec.md` 变更记录
