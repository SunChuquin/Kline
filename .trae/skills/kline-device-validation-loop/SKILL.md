...
- 中文提交信息，格式 `<type>(<scope>): <简述> <详细描述>`，如 `feat(link-replay A): ...`、`fix(link-replay): ...`、`docs: ...`；其中 `<详细描述>` 需要
...
说清2件事：①改了什么、为什么（bug 要讲根因，最好复现推导链）；②请在真机上**具体验证什么路径**（给出操作步骤与预期现象，包括回归点：确认旧功能未受影响）。
