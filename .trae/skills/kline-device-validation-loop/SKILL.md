...
3. **每阶段固定五步**：编码自查 → 提交推送 → **等 CI 成功** → 通知部署助手 → **明确告知用户本轮验收点，然后停下等真机结论**，不要自行推进下一阶段。设备就绪门禁由**部署助手**在下载 IPA 前自动执行（30s 窗口探测设备 KlineHTTP :5051，不响应则判定无人值守、不下载不安装、助手进入"设备无人值守"终态，脚本据其映射为退出码 6）。遇到退出码 6 视为"人不在设备面前"：提示用户解锁 iPad、打开 Kline 保持前台，然后凭 run_id 走下方手动链路 `POST :5052/notify` 续跑部署即可（构建产物已在，无需重新提交/重新构建）。
...
- **一键端到端**：`python c:\Users\sunck\home\projects\ios\TrollRestore\build_and_deploy.py "<提交描述>"`（在 Kline 仓库根执行）
  - 内部完成 add→commit→push（含 rebase 重试）、按 headSha 匹配本次 run、轮询构建、失败自动拉日志摘录 error 行（退出码 1）；构建成功即 `POST :5052/notify {run_id}` 通知助手并等部署终态——**设备就绪门禁在助手侧执行**（见下），脚本不碰 usbmux forward；
  - 部署助手离线时脚本会用 `.venv-ios\Scripts\pythonw.exe` 后台自动拉起，无需手动开 GUI；
  - 退出码：0 成功 / 1 构建失败 / 2 git 失败 / 3 助手无法启动 / 4 超时 / 5 助手忙 / **6 设备无人值守**。
  - **退出码 6 = 人不在设备面前，构建已完成、等待后续人工处理**：CI 构建成功且已通知助手，但助手在**下载 IPA 前**的 30s 门禁窗口内探测不到设备 KlineHTTP（:5051）——iPad 锁屏 / Kline 未在前台 / USB 断连（结果 JSON 含 `run_id`、`assistant_status`，stage=device-gate）。助手此时**不下载 IPA、不安装**；处理方式：请用户解锁 iPad 并打开 Kline 保持前台，AI 再凭 `run_id` 用下方手动链路的 `POST :5052/notify {"run_id":<RUN_ID>}` 续跑（无人值守终态对助手算"空闲"，重新通知会被接受），已成功的构建产物直接复用，无需重新提交。门禁时长常量在 `deploy_gui.py` 的 `DEVICE_READY_SECONDS`（30）。
  - 注意（真机实测结论，别再走回头路）：**afc 在锁屏下仍可正常列目录/传文件**（配对关系的 escrow bag 使其不受锁屏限制），无法用来判别锁屏；门禁信号只能依赖"Kline 在前台才在线"的 KlineHTTP。门禁历史上曾放在 build_and_deploy.py 里自开 forward 探测，因与助手重复持有 5051 转发（要 kill_port 善后）、且锁屏时白下载 IPA，已移至 `deploy_gui.py` 的 DeployWorker（手动 notify 与 autopoll 两个入口统一生效）。
