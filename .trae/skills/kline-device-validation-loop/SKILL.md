...
- 用户按真机反馈提出调整或报 bug，例如“我感觉”或者“我发现”之类的，就需要"改→装→验"快速循环。

不适用：用户明确说"先别改/只评估"的场景（此时只产出分析或计划文档）。
...
3. **执行当前阶段**：编码自查 → **闭环命令（强制要求：使用前台阻塞方式执行）** → **当退出码等于"0 成功"、"6 云端构建成功，但设备无人值守" 或 "退出码 7：GitHub API 轮询连试 3 次仍失败（瞬时网络故障）**"时，应交付给用户；可若是其它退出码则需要AI自行规划解决 → **明确告知用户本轮验收点，然后停下等真机结论**，不要自行推进下一阶段，如果用户没有明确要求更新**项目记忆**就不要主动更新。
4. **用户确认后**：再进入下一阶段；全部完成后更新计划文档状态为"已验收"；
...
- 中文提交信息，格式 `<type>(<scope>): <简述> <详细描述>`，如 `feat(link-replay A): ...`、`fix(link-replay): ...`、`docs: ...`；其中 `<详细描述>` 需要明确说清三件事：①本轮装的是哪个 build；②改了什么、为什么（bug 要讲根因，最好复现推导链）；③请在真机上**具体验证什么路径**（给出操作步骤与预期现象，包括回归点：确认旧功能未受影响）。

## 闭环命令（强制要求：使用前台阻塞方式执行）

用户的缺省要求是提交当前仓库全部改动，即执行以下命令：
