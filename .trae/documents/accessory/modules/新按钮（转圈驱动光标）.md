...
flowchart TB
    ...
    C --> E[摁住/拖动整个按钮<br/>点击：清全部光标 + 窗口平移 1 根]
...
stateDiagram-v2
    ...
    s2 --> s0: 自己被触碰
    s0 --> s3: 点击（位移 ≤ 6pt）
    s3 --> s0: 点击：先清全部视图所有光标，再把窗口朝更新方向平移 1 根
    s0 --> s4: 另一方开始手势
    s4 --> s0: 自己被触碰
```
...
- `s2` 与 `s4` 外观相同但**来源不同**：`s2` 是自己 0.5 秒没被碰，`s4` 是被另一方强制；
  **闲置时长两按钮不同**：旧按钮 3 秒（`idleFadeDelay`）、新按钮 0.5 秒（`wheelIdleFadeDelay`）；
- `s4` **不会**因为另一方手势结束而自动退出（单向耦合，见 [[20260919-手势期间另一方强制闲置]]）；
- **图里没画的一层外观切换**：`s1` 期间若是**从 B' 起手**（整钮模式），整个按钮会临时换成**旧按钮拖动状态的外观**，
  并一直保持到 `s2` 那一刻（与闲置淡出同一时刻还原）。从环 Z' 起手则用新按钮自身外观 ——
  见 [[20260919-B'被操作期间整体换成旧按钮外观]]。

**调用关系**：`ContentView` → `FloatingAccessoryWheel()`（不需要注入 action）。出口都在协调对象上：

- **环上转圈** → `advanceCursor(by:)` → 主格 `KlineChartView` 的 `.onReceive($cursorAdvance)` → `applyAccessoryCursorAdvance`
- **点击 B'** → 两步：`clearAllCursors()` → **`KlineDetailView`** 的 `.onReceive($clearAllCursorsSeq)`（清全部格的光标）；
  然后 `nudgeWindow(by:)` → 主格 `.onReceive($windowNudge)` → `applyAccessoryWindowNudge`

转圈与平移两条出口都只由**主格**（`isMainTile`）消费、都带 `seq` 去重；清光标那条则**有意不限主格**
（要清的就是屏幕上所有格），由持有 `cursorClearToken` 与 `linkSync` 的详情页统一执行。空转期三者都零写入。
...
- [[20260919-B'被操作期间整体换成旧按钮外观]] — B' 起手时整体临时变旧按钮外观，静止 0.5 秒还原
- [[20260919-点击B'平移窗口一根]] — 点击 B' = 先清全部视图所有光标，再把被驱动那一格的窗口朝更新方向平移 1 根

## 3. 已知陷阱

- [[20260919-Published同值赋值也会发布]] — 高频 `onChanged` 里给 `@Published` 赋值会造成发布风暴
- [[20260919-手势宿主不能在自身位移之内]] — 手势必须挂在 `.position` **之后**，否则拖动只走到手指的一半
