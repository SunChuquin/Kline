...
- `D` 除写日线外，还会用「**主库当期 bar ⊕ 新日线**」合并出**当期季/年 bar**，与日线**同一事务**写进 `live_quarterly` / `live_yearly`（`WatchlistSyncManager.currentPeriodBars` / `basePeriodBars` / `mergePeriodBar`，见 [[20260923-行情同步-五张周期表全路径一致性]]）。当期 bar 是**原地更新**（不产生更大日期），所以别用 `MAX(date)` 判断它有没有更新。
