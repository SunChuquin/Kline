...
| 层 | 文件 | 内容 | 更新方式 |
| :--- | :--- | :--- | :--- |
| 主库 | `Documents/tdx.db` | 全市场 3611 只、日线 1400 万行、1.38GB | 整库替换需重启（兜底）；**主路径用 `POST /sync/apply-patch` / `patch-session` 按行 UPSERT、免重启**（见 [[20260923-行情同步-补丁按行落主库而非替换整库]]） |
| 增量库 | `Documents/tdx_live.db` | 按 `(file, date)` 存的新行，**发布五张周期表**（日/周/月/季/年），**查询时优先于主库** | 自动（三条通道） |
...
flowchart LR
    ...
    subgraph PC[PC 侧 · 按需]
        E[txt_patch_builder<br/>新 txt vs 基线库出差分] --> F[sync_pipeline<br/>分片并行 + 会话落主库]
        F --> G[apply-patch/patch-session<br/>写主库 + 清增量库 + reload]
    end
    H[东财 push2 接口] --> B
    I[vipdoc / 前复权 txt] --> E
    J[主库 tdx.db] --> E
...
- PC 侧那条链**没有定时**，由用户在通达信客户端补完缺口后手动跑（主路径一条 `sync_pipeline.py` 走完出包→PUT→落库→清增量库→reload；旧路径 `tdx_parser`→`diff_live_patch`→`push_bucket_usb` 三步）。
- 补丁走的是**主库按行 UPSERT**（`patch-session` 会话式单事务）；分片（`G` 的另一支）仍走增量库 `mergeBucket`，两者对包内日期都不做校验，差别只在目的地与包内日期可跨任意历史。
...
| **PC 出差分包推设备**（主路径） | 用户手动 | `src/txt_patch_builder.py` → `TrollRestore/sync_pipeline.py`（分片并行出包 + 会话式单事务落主库 + 清增量库 + reload）<br/>旧路径：`tdx_parser.py --rebuild`/`--prev-dir` → `diff_live_patch.py` → `push_bucket_usb.py --apply-main` |
...
| 拿 `tdx_project/tdx.db` 当差分基线 | 该库**内部** `meta` 被重编号（连续 1..3611）而数据表是 3..3637，两套号错位 → 按 `file` 关联产 1231 万行假差异（注：其数据表与主库同号且值差异为 0） | `diff_live_patch.py` 的 `check_meta_consistency` 实测拦下 |
...
| 差分生成耗时 | **7.0 秒**（原 **400.0 秒**；三层优化：扫描形状 + 变更裁剪 + 固定开销，见 [[复权差分重灌通道]] §9） | 1.4GB 库对、热缓存、`stat` 变更判定、0 候选。严格档 `--verify-hashes` 为 57.4s，全量比对 `--no-prune` 冷缓存 419.2s |
...
关键换算：**每天要传的量从 1.38GB 降到约 10MB（1/140）**，代价是 PC 侧每次出包约 15~25s（盘后任务，可接受；端到端 57.8s → 32.7s，见 [[复权差分重灌通道]] §9）。
...
- [[20260922-行情同步-关联键用file而非meta_id]] — id 同谱系内其实稳定；选 file 是因为它是端到端契约键
...
- [[20260923-行情同步-补丁按行落主库而非替换整库]] — 补丁走主库按行 UPSERT（不需要重启 App）
- [[20260923-行情同步-五张周期表全路径一致性]] — 五张周期表在所有更新路径上都要最新
...
- [[20260923-活跃事务内DETACH必失败]] — 会话事务未结束就 DETACH 会报错
- [[20260923-本机HTTP代理导致本地下载假503]] — 本机代理会把「下载失败」伪装成 503
- [[20260923-wal_autocheckpoint在COMMIT内吃掉十几秒]] — 默认 autocheckpoint 在 COMMIT 内做全量回写
- [[20260923-通达信txt末尾18字节页脚导致纯追加被误判]] — 末尾 18 字节页脚会让「纯追加」被误判成改写
